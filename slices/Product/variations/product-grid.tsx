...
export async function ProductGrid({ slice }: Props) {
  ...
  const { overline, title, description, alignment, button, section_theme, remove_top_padding, featured_products } =
    slice.primary;
  ...
  const curated = (
    await Promise.all(
      featured_products.map((item) =>
        isFilled.contentRelationship(item.product) ? client.getByID<ProductDocument>(item.product.id) : null,
      ),
    )
  ).filter((product): product is ProductDocument => product !== null);

  const products = curated.length > 0 ? curated : await client.getAllByType("product");
