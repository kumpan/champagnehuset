...
type Props = EmployeeProps & { slice: Content.EmployeeSliceList };
...
export async function EmployeeList({ slice }: Props) {
        ...
        {employees.length > 0 && (
          <div className="mt-8 grid grid-cols-2 gap-3 md:grid-cols-3 md:gap-4 lg:grid-cols-4">
            {employees.map((employee) => (
              <EmployeeCard key={employee.id} employee={employee} sectionTheme={section_theme} className="w-full" />
            ))}
          </div>
        )}
      </Container>
